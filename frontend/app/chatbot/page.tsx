...
export default function ChatbotPage() {
  ...
  useEffect(() => {
    console.log("🚀 Chatbot Configured API URL:", API_BASE_URL);
    fetchSessions();
  }, []);
  ...
  const fetchSessions = async () => {
    try {
      console.log(`Fetching sessions from: ${API_BASE_URL}/chat/sessions/${userId}`);
      ...
      if (res.ok) {
        ...
        // Only alert on production to avoid annoying dev flow
        if (process.env.NODE_ENV === 'production') {
          alert(`Error connecting to server (${res.status}). Check console for details.`);
        }
      }
    } catch (err) {
      console.error("Network error fetching sessions:", err);
      // alert("Network Error: Could not connect to backend. Please check your internet or server status.");
    }
  };
}
  };
